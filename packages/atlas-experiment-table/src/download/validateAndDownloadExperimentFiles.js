...
export default async (host, experimentAccessions, fileTypes) => {
  try {
    ...
    if (invalidFiles.length === 0 ||
      ...
      window.location.replace(
        URI(`experiments/download/zip`, host)
          .search({ accession: experimentAccessions, fileType: fileTypes }).toString())
    }
  } catch (e) {
    console.error(e)
  }
